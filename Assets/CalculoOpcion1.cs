...
public class CalculoOpcion1 : MonoBehaviour
{
    ...
    //Variable que almacena la direccion actual del punto
    private int ndir = 0;
    ...
    private void moveError(int dir){
        int n = UnityEngine.Random.Range(0,2);
        float direct1 = (float) directions[dir];
        float direct2 = (float) directions[(dir+4)%directions.Length];
        float [] options = {direct1, direct2};
        float theta = options[n];
